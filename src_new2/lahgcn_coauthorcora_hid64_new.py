...
for i in trange(args.runs, desc='Run Train'):
    ...
    for epoch in range(args.epochs):
        ...
        if loss_val < best:
            ...
            torch.save(best_model.state_dict(), 'lahgnn_coauthorcora_hid64_0108.pth')
    ...
    # 保存到文件
    np.savetxt('res/lahgnn_predicted_coauthorcora_hid64_0108.txt', predicted_array, fmt='%d')
...
# # 从文件中读取预测结果
# predicted_array = np.loadtxt('res/lahgnn_predicted_coauthorcora_hid64_0104.txt', dtype=int)
# # 将numpy数组转换为列表
...
# plt.savefig('figs/lahgnn_true_predic_coauthorcora_hid64_0104.pdf')