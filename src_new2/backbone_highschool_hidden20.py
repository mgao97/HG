...
for run in range(5):
    net = HGNN(X.shape[1], 20, num_classes, use_bn=True)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_acc > best_val:
                ...
                # torch.save(net.state_dict(), 'model/hgnn_highschool_best_model_0117.pth')
        # scheduler.step()
    print("\ntrain finished!")
...
new_X, new_lbls = X.to(device), lbls.to(device)
new_G = G.to(device)
...
all_acc, all_microf1, all_macrof1 = [],[],[]
for run in range(5):
    net = HyperGCN(new_X.shape[1], 20, num_classes)
    optimizer = optim.Adam(net.parameters(), lr=0.01, weight_decay=5e-4)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_acc > best_val:
                ...
                best_val = val_acc
                best_state = deepcopy(net.state_dict())
                # torch.save(net.state_dict(), 'model/hypergcn_highschool_best_model_0117.pth')
        # scheduler.step()
    print("\ntrain finished!")
    print(f"best val: {best_val:.5f}")
...
for run in range(5):
    ...
    model_unigin = UniGIN(X.shape[1], 20, num_classes, use_bn=True)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_acc > best_val:
                ...
                # torch.save(model_unigin.state_dict(), 'unigin_highschool_best_model_0117.pth')
...
for run in range(5):
    ...
    model_unisage = UniSAGE(X.shape[1], 20, num_classes, use_bn=True)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_acc > best_val:
                ...
                torch.save(model_unisage.state_dict(), 'unisage_highschool_best_model_0117.pth')
