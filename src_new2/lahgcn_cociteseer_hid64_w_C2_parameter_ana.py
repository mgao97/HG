...
def add_gaussian_noise(tensor, mean=0, std=0.001):
...
def get_augmented_features(concat):
    ...
    for _ in range(concat):
        ...
        augmented_features = add_gaussian_noise(cvae_features, mean=0, std=0.005)
        augmented_features = hgnn_cvae_pretrain_new_citeseer.feature_tensor_normalize(augmented_features).detach()
        if args.cuda:
            X_list.append(augmented_features.to(device))
        else:
...
for i in trange(args.runs, desc='Run Train'):
    ...
    for epoch in range(args.epochs):
        ...
        if loss_val < best:
            ...
            torch.save(best_model.state_dict(), 'lahgcn_cociteseer_hid64_C2_hidden32_0112.pth')
    ...
    # 保存到文件
    # np.savetxt('res/lahgnn_cociteseer_hid64_ablation_0105.txt', predicted_array, fmt='%d')
...
print('Parameter analysis with C2 on CocitationCiteseer dataset:')
print('\n')
...
for i in trange(args.runs, desc='Run Train'):
    ...
    for epoch in range(args.epochs):
        ...
        if loss_val < best:
            ...
            best_model = copy.deepcopy(model)
            best_X_list = copy.deepcopy(val_X_list)
            torch.save(best_model.state_dict(), 'lahgcn_cociteseer_hid64_C2_hidden128_0112.pth')
...
for i in trange(args.runs, desc='Run Train'):
    ...
    for epoch in range(args.epochs):
        ...
        if loss_val < best:
            ...
            best_model = copy.deepcopy(model)
            best_X_list = copy.deepcopy(val_X_list)
            torch.save(best_model.state_dict(), 'lahgcn_cociteseer_hid64_C2_hidden256_0112.pth')
