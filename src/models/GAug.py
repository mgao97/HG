...
from hgcn.layers import HGNNConv

class HGAug(object):
    def __init__(self, adj_matrix, features, labels, tvt_nids, cuda=-1, hidden_size=128, emb_size=32, n_layers=1,
                 epochs=200, seed=-1, lr=1e-2, weight_decay=5e-4, dropout=0.5, hgae=False, beta=0.5, temperature=0.2,
                 log=True, name='debug', warmup=3, gnnlayer_type='hgcn', jknet=False, alpha=1, sample_type='add_sample',
                 feat_norm='row'):
        self.lr = lr
        ...
        # setup the model
        self.model = HGAug_model(
            dim_feats=self.features.size(1),
            dim_h=hidden_size,
            dim_z=emb_size,
            n_classes=self.out_size,
            n_layers=n_layers,
            activation=F.relu,
            dropout=dropout,
            device=self.device,
    ...
    def load_data(self, hypergraph, labels, tvt_nids, gnnlayer_type):
        """Preprocess data for the hypergraph"""
        # features (torch.FloatTensor)
        if isinstance(hypergraph.features, torch.FloatTensor):
            self.features = hypergraph.features
        else:
            self.features = torch.FloatTensor(hypergraph.features)
        ...
        # original hypergraph for training vgae (torch.FloatTensor)
        self.hypergraph = hypergraph
        adj_matrix = hypergraph.get_incidence_matrix()
        adj_matrix.setdiag(1)
        self.adj_orig = adj_matrix.to_dense()

        # normalized hypergraph used as input for ep_net (torch.sparse.FloatTensor)
        degree_mat_inv_sqrt = hypergraph.get_degree_matrix_inv_sqrt()
        ...
        if gnnlayer_type == 'gcn':
            ...
            adj_matrix_noselfloop = adj_matrix.copy()
            adj_matrix_noselfloop.setdiag(0)
            adj_matrix_noselfloop.eliminate_zeros()
            adj_matrix_noselfloop = adj_matrix_noselfloop / adj_matrix_noselfloop.sum(1)
    ...
    def fit(self, pretrain_ep=200, pretrain_nc=20):
        """Train the model on the hypergraph."""
        # Move data to device
        ...
        optims = MultipleOptimizer(
            torch.optim.Adam(model.ep_net.parameters(), lr=self.lr),
            torch.optim.Adam(model.nc_net.parameters(), lr=self.lr, weight_decay=self.weight_decay)
        )
        ...
        for epoch in range(self.n_epochs):
            ...
            if val_acc > best_val_acc:
                ...
                self.logger.info(
                    'Epoch [{:3}/{}]: ep loss {:.4f}, nc loss {:.4f}, ep auc: {:.4f}, ep ap {:.4f}, val acc {:.4f}, test acc {:.4f}'
                    .format(epoch + 1, self.n_epochs, ep_loss.item(), nc_loss.item(), ep_auc, ep_ap, val_acc, test_acc))
                patience_step = 0
            else:
                self.logger.info(
                    'Epoch [{:3}/{}]: ep loss {:.4f}, nc loss {:.4f}, ep auc: {:.4f}, ep ap {:.4f}, val acc {:.4f}'
    ...
    def pretrain_ep_net(self, model, adj, features, adj_orig, norm_w, pos_weight, n_epochs):
        ...
        optimizer = torch.optim.Adam(model.ep_net.parameters(), lr=self.lr)
        model.train()
    ...
    def pretrain_nc_net(self, model, adj, features, labels, n_epochs):
        ...
        optimizer = torch.optim.Adam(model.nc_net.parameters(), lr=self.lr, weight_decay=self.weight_decay)
...
# 用于端到端的图神经网络训练的类，包括节点分类和连边预测，并提供一些额外的功能，如预训练、学习率调整、日志记录等
class GAug(object):
    def __init__(self, adj_matrix, features, labels, tvt_nids, cuda=-1, hidden_size=128, emb_size=32, n_layers=1, epochs=200, seed=-1, lr=1e-2, weight_decay=5e-4, dropout=0.5, hgae=False, beta=0.5, temperature=0.2, log=True, name='debug', warmup=3, gnnlayer_type='gcn', jknet=False, alpha=1, sample_type='add_sample', feat_norm='row'):
        self.lr = lr
        self.weight_decay = weight_decay
        self.n_epochs = epochs
        self.hgae = hgae
...
class HGAug_model(nn.Module):
    def __init__(
        ...
        self.ep_net = VHGAE(dim_feats, dim_h, dim_z, activation, hgae=hgae)

        # node classification network
        if jknet:
            self.nc_net = HGNN_JK(dim_feats, dim_h, n_classes, n_layers, activation, dropout)
        else:
            self.nc_net = HGNN(dim_feats, dim_h, n_classes, n_layers, activation, dropout)

    def sample_adj(self, adj_logits, hg):
        edge_probs = adj_logits / torch.max(adj_logits)
        adj_sampled = hg.sample_adj(edge_probs, temperature=self.temperature)
        return adj_sampled

    def sample_adj_add_bernoulli(self, adj_logits, adj_orig, alpha, hg):
        edge_probs = adj_logits / torch.max(adj_logits)
        edge_probs = alpha * edge_probs + (1 - alpha) * adj_orig
        adj_sampled = hg.sample_adj(edge_probs, temperature=self.temperature)
        return adj_sampled

    def sample_adj_add_round(self, adj_logits, adj_orig, alpha, hg):
        edge_probs = adj_logits / torch.max(adj_logits)
        edge_probs = alpha * edge_probs + (1 - alpha) * adj_orig
        adj_sampled = hg.sample_adj_round(edge_probs)
        return adj_sampled

    def sample_adj_random(self, adj_logits, hg):
        edge_probs = adj_logits / torch.max(adj_logits)
        adj_sampled = hg.sample_adj_random(edge_probs)
        return adj_sampled

    def sample_adj_edge(self, adj_logits, adj_orig, change_frac, hg):
        edge_probs = adj_logits / torch.max(adj_logits)
        adj_sampled = hg.sample_adj_edge(edge_probs, adj_orig, change_frac)
        return adj_sampled

    def normalize_adj(self, adj, hg):
        # Use the smoothing_with_HGNN method from Hypergraph to normalize the adjacency matrix
        adj_normed = hg.smoothing_with_HGNN(adj)
        return adj_normed

    def forward(self, adj, adj_orig, features, hg):
        adj_logits = self.ep_net(adj, features, hg)

        if self.sample_type == 'edge':
            adj_new = self.sample_adj_edge(adj_logits, adj_orig, self.alpha, hg)
        elif self.sample_type == 'add_round':
            adj_new = self.sample_adj_add_round(adj_logits, adj_orig, self.alpha, hg)
        elif self.sample_type == 'rand':
            adj_new = self.sample_adj_random(adj_logits, hg)
        elif self.sample_type == 'add_sample':
            if self.alpha == 1:
                adj_new = self.sample_adj(adj_logits, hg)
            else:
                adj_new = self.sample_adj_add_bernoulli(adj_logits, adj_orig, self.alpha, hg)

        adj_new_normed = self.normalize_adj(adj_new, hg)
        nc_logits = self.nc_net(adj_new_normed, features, hg)
...
class VHGAE(nn.Module):
    '''VHGAE as edge prediction model'''
    def __init__(self, dim_feats, dim_h, dim_z, activation, hgae=False):
        super(VHGAE, self).__init__()
        self.hgae = hgae
        self.hgcn_base = HGNNConv(dim_feats, dim_h, aggregator_type='sum')
        self.hgcn_mean = HGNNConv(dim_h, dim_z, aggregator_type='sum')
        self.hgcn_logstd = HGNNConv(dim_h, dim_z, aggregator_type='sum')  # 替换为你的超图卷积层

    def forward(self, hg, features):
        # HGNN encoder
        hidden = self.hgcn_base(features, hg)
        self.mean = self.hgcn_mean(hidden, hg)
        ...
        if self.hgae:
            ...
            # VGAE
            self.logstd = self.hgcn_logstd(hidden, hg)
...
class HGNN_JK(nn.Module):
    def __init__(self, dim_feats, dim_h, n_classes, n_layers, activation, dropout, gnnlayer_type='hgcn'):
        super(HGNN_JK, self).__init__()
        heads = [1] * (n_layers + 1)
        if gnnlayer_type == 'hgcn':
            gnnlayer = HGNNConv
        elif gnnlayer_type == 'gsage':
            gnnlayer = HGNNConv  # 使用你的超图卷积层
        elif gnnlayer_type == 'gat':
            gnnlayer = HGNNConv  # 使用你的超图卷积层
            heads = [8] * n_layers + [1]
            dim_h = int(dim_h / 8)
        ...
        self.layers = nn.ModuleList()
        # input layer
        self.layers.append(gnnlayer(dim_feats, dim_h, dropout=dropout))
        # hidden layers
        for i in range(n_layers - 1):
            self.layers.append(gnnlayer(dim_h, dim_h, dropout=dropout))
        # output layer
        self.layer_output = nn.Linear(dim_h * n_layers, n_classes)
