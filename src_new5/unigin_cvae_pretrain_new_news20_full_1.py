...
from dhg.nn import HGNNConv,UniGINConv
...
def get_augmented_features(args, hg, features, labels, idx_train, features_normalized, device):
    ...
    model = UniGIN(in_channels=features.shape[1], hid_channels=hidden, num_classes=labels.max().item()+1, use_bn=False, drop_rate=dropout)
