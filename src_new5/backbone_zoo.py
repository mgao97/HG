...
G = Hypergraph(data.x.shape[0], he_list)
print('hg:',G)
...
num_classes = len(list(torch.unique(lbls).numpy()))
...
# net = HGNN(X.shape[1], 8, num_classes, use_bn=True)
...
he = G.e[0]
G.e[0][27] = (0, num_vertices)
print(G.e[0][27])
new_G = Hypergraph(102, he)
lbls = data.y

print(lbls.shape, lbls[0])
...
new_X = torch.cat((X, X[16].unsqueeze(0)),dim=0)
# print(new_X.shape)

num_vertices = new_X.shape[0]

new_lbls = torch.cat((lbls, torch.LongTensor([1])), dim=0)
new_lbls = new_lbls - torch.min(new_lbls)
...
all_acc, all_microf1, all_macrof1 = [],[],[]
net = HyperGCN(new_X.shape[1], 8, num_classes)
optimizer = optim.Adam(net.parameters(), lr=0.01, weight_decay=5e-4)
...
# num_epochs = 200
# all_acc, all_microf1, all_macrof1 = [],[],[]
# model_unigin = UniGIN(X.shape[1], 8, num_classes, use_bn=True)
...
#     model_unisage = UniSAGE(X.shape[1], 8, num_classes, use_bn=True)
