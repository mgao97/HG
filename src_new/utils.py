...
# # def normalize_adj(hg):
# #     '''symmetrically normalize hypergraph adjacency matrix.'''
# #     adj_sparse = hg.H * hg.H.T  # Assuming adjacency_matrix() returns the hypergraph adjacency matrix
    
# #     # Convert to COO format
# #     # adj_coo = adj_sparse.coalesce().to('cpu')
# #     adj_coo = adj_sparse.to('cpu')
# #     adj_coo += sp.eye(adj_coo.shape[0]) 
# #     row, col, values = adj_coo.row(), adj_coo.col(), adj_coo._values()
    
# #     rowsum = torch.sparse.sum(adj_sparse, dim=1).to_dense()
# #     d_inv_sqrt = 1.0 / torch.sqrt(rowsum)
# #     d_inv_sqrt[d_inv_sqrt == float('inf')] = 0.
    
# #     d_mat_inv_sqrt = torch.diag(d_inv_sqrt)
    
# #     normalized_values = spspmm(row, col, values, row, col, d_inv_sqrt * values)

# #     # Get num_v (number of vertices)
# #     num_v = max(torch.max(row), torch.max(col)) + 1
    
# #     # Get e_list (edge list)
# #     e_list = torch.cat((row.view(-1, 1), col.view(-1, 1)), dim=1).tolist()

    
# #     # Assuming you want to convert it back to a DHGraph
# #     normalized_hg = Hypergraph(num_v=num_v, e_list=e_list)
